...
def populate_size_scores(db_path: str) -> None:
    ...
    try:
        with conn:
            ...
            per_uid: DefaultDict[int, DefaultDict[str, List[Tuple[int, int, Optional[int]]]]] = defaultdict(
                lambda: defaultdict(list)
            )

            rows = conn.execute(
                """
                SELECT uid, model, size, borderline, count(*) AS cnt
                FROM results_w_model
                GROUP BY uid, model, size, borderline
                """
            )
            ...
            for row in rows:
                ...
                per_uid[uid][model].append((cnt, size, borderline_other))

            for uid, data in per_uid.items():
                ...
                conn.executemany(
                    """
                    INSERT INTO model_size_scores (uid, model, size, lower, higher, s_60, s_70, s_80, s_99)
                                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    ((uid, model, sz, float(lower), float(higher), *(float(m) for m in mass))
                     for model, (sz, lower, higher, mass) in outputs.items()))

                sz, reason = determine_final_size(outputs)
                conn.execute(
                    """
                    INSERT INTO final_size_scores (uid, size, reason, excl, veto_60, score_60, score_70)
                                           VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (uid, sz, str(reason), reason.excl, reason.veto_60, float(reason.score_60), float(reason.score_70)))
    finally:
        conn.close()
